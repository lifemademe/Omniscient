...
| **T-4** | Tooling | A capture POSE inside a surface, not just the entrance | — | `OPEN` | **Now the top blocker for every remaining warehouse item.** `jump.py W` always lands at the inbound dock, synthetic keys do not reach the game so the drone cannot be flown, and TAB does not change the camera. W-1 spent nine rounds judged from the one part of the building with no high bays in it. W-2, W-3 and W-4 all need an interior viewpoint too. A dev-gated capture pose is the same shape of thing as SceneJump and belongs beside it |
