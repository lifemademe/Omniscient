...
| **W-2** | Warehouse | Depth cueing — value + saturation falloff with distance | §4.4 | `IN LOOP` claude 2026-08-27 | **The fog was the outdoor numbers with a warehouse name on them.** near 32 / far 120 on a 58m building = zero haze for the first 32m and 26% at the rear wall, so the cue was switched off across the whole space the player occupies; measured, saturation ROSE with distance (0.229 → 0.408). Now 9 / 62, and the haze colour taken from `#5d6b77` (luma 105, a daylight value) to `#141b23` — a night interior has no sky, so distance must fade DOWN. Saturation now falls 0.372 → 0.331 across a long view. `W2-r1-fog.jpg`. Awaiting the critic |
