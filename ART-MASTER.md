...
| **W-2** | Warehouse | Depth cueing — value + saturation falloff with distance | §4.4 | `IN LOOP` claude 2026-08-27 | The fog was the outdoor numbers with a warehouse name on them: near 32 / far 120 on a 58m building = **zero haze for the first 32m**, and saturation ROSE with distance. Now 9 / 72 with the haze taken from `#5d6b77` (luma 104, a daylight value — outdoor haze is bright because the sky lights it) to `#1e2732`. r1 overshot to `#141b23` and a critic read it as *"deep but bottomless"*; **the haze luma is the FLOOR of the ramp**, so raising it 26 → 38 took the far end's p5 from 7 to 16 while its spread held at ~140. `W2-r2-fog.jpg` |
