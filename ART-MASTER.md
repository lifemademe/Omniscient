...
| **W-1c** | Warehouse | Real darkness between the pools | `warehouse/02-high-bay-pools.jpg` | `PAUSED` claude 2026-08-26 | **Twelve rounds; stopped deliberately, not from budget.** The room went from median 114 / p95 223 / 0.4% clipping to within a level or two of the bar on median and p5 with no clipping, and reads as a night interior. Four critics named the same unclosed gap: no lit patch is visibly OWNED by a fitting. Two suspects were eliminated with evidence — the cel pass (A/B'd, floor profiles identical) and shadow maps (a pool is falloff, not occlusion; the room already has a 2048 directional). **The likely cause is the camera, not the lights:** an 11.6m pool at 30m is compressed by foreshortening into a few pixels of depth, and the bar is shot from a viewpoint the drone cannot occupy. Resume only with a bar the game's camera can reach |
| **W-2** | Warehouse | Depth cueing — value + saturation falloff with distance | §4.4 | `OPEN` | **Read W-1 first.** The floor is not where this room's light can be read: from the drone at head height down a 48m building the floor is so foreshortened that an 11.6m pool at 30m compresses to a few pixels of depth. The camera spends its time on RACK FACES. Cadence authored across the bays — lit bay, dim bay — is likely to buy more than any further work on the floor, and it is Law 3 as well |
| **W-3** | Warehouse | A lamp on the drone that lights what it approaches | §4.4 | `BLOCKED` by W-1 | Lamp retuned 4.5/11/1.35 → 7.5/9/1.5 and moved 12cm off the nose, which recovers the range that was spent compensating for an over-bright room and holds hull clipping flat (0.33%, p95 196→199). **It delivers no visible approach signal and the reason is not the lamp.** Parked 1.2m off a rack face, an identical carton measures 162 near against 164 far — 0.99x. Rack contents sit near the top of the tone curve, so 4 units of extra radiance land on the flat part and produce nothing. Same root as W-1 from the other side: **the building has no local darkness for a local light to work against.** Capture `scripts/dev/W3-approach.jpg` |
