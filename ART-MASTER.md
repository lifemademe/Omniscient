...
| **D-4** | Dioramas | One shadow-casting key per room | §5 | `IN LOOP` claude 2026-08-27 | **Audit: exactly ONE of nine builders had a shadow-casting key.** Eight rooms, 24 lights, nothing casting. Repair shop took **four rounds and three critic rejections**: the face spot cast only onto the wall behind her (radio moved 0.01 levels); the bench practical bought contact and threw the fixture's own silhouette over a third of the frame; a `noShadowCast` opt-out in `applyShadowPolicy` fixed that but exempted the whole prop, so the lamp BASE floated beside a radio that no longer did; splitting a `pedestal` part off `PropParts` lets the base cast while the shade and arm stay exempt. **Two rules that generalise to the other seven rooms:** a key that cannot reach the surface objects rest on cannot create contact however strong it is; and a fixture depicting a light must not occlude it — but exempt the shade, not the stand. Seven rooms still uncast |
