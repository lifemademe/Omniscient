...
| **M-1** | M4SS | Stage 3 (Sluice) full art pass | M4SS-ART-BIBLE §2 | `OPEN` | **"Grey-boxed" was wrong** - rendered at `captures/m4ss-go.png`, stage 3 already has lit platform lips, vertical god-ray shafts, lanterns with warm motes, warm hazards against the green, hanging vines, textured undersides. Confirmed as the Sluice by two theme markers (gold motes, vertical light). Remaining against §2: **no vessels / bell jars, no dome lattice, standing water reflects nothing**. Do NOT brighten the midground - tried and reverted twice, reasons at stageArt.ts:1162 |
...
| **D-1** | Dioramas | Eight rooms, eight named lighting looks | §4.3 / §7 | `IN LOOP` | Surveyed all 8 in greyscale (`captures/d1-before/sheet-grey.png`). 3/4/5/6/8 already read as distinct looks; **repair-shop was the flattest lit room in the game** and is now done - "bench pool", `captures/d1-after/1-final.png`. **mill-road renders at 98.9% black** - see D-1a |
| **D-1a** | Dioramas | Mill road renders black | §4.3 | `OPEN` | Every prop is claimed by `createTorchlight`; boosted 14x the frame is empty but for the guess-lattice specks, so the beam never lights and `GUESS_FILL` #0a141a lands at ~0. Not yet known whether this is preview-only (the torch's `aim()` runs from an idle hook) or the live mission too |
...
| **C-1** | Workstation | Air: dust in the lamp cone, flicker, settle | §4.1 concept frame | `IN LOOP` | Cone + falling dust + filament wobble built (`art/lampCone.ts`), evidenced at `captures/c1-compare.png`. The cone is proved; the 2% wobble and the settle rate are NOT - the menu camera drifts, so per-frame sampling cannot separate them from the move |
| **C-2** | Workstation | The room ages across nine missions | §4.1 | `IN LOOP` | The room ages by **running out of daylight** - afternoon → sunset → night, one step per request answered, and the town across the water comes on as the sky goes. Three stops captured at `captures/c2-arc.png`, window measured (112,117,110) → (20,30,48). Forced-value captures; the progression has not been watched across a real playthrough |
