...
| **A-1** | Access | Audit every hue-carried mechanic | §11 | `IN LOOP` claude 2026-08-27 | **Found and fixed a real violation, and the rule now has a harness.** The globe carried four signal states in hue alone; desaturated, the ANOMALY sat **7.6 levels** from a contact the player had already helped (106.9 vs 99.3), and `cooldown`/`unknown` shared a hex outright. Rebuilt as a value ladder — waiting 185 / unknown 132 / cooldown 111 / resolved 78, closest pair 20.3 — plus the anomaly is bracketed, so it differs in KIND not degree (serves C-4). **New gate: `scripts/law5-states.ts`**, canaried against the original bug. Five other surfaces audited clean and listed as EXEMPT in it, each naming its non-hue channel. Still to audit: console, dioramas |
