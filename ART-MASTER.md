...
| **D-1** | Dioramas | Eight rooms, eight named lighting looks | §4.3 / §7 | `AT BAR (7 of 8)` | **Bar: with colour removed, each room is identifiable by the SHAPE of its light alone.** Seven of eight pass on `captures/d1-now/sheet-grey.png`: 1 bench pool, 2 single bulb, 3 beacon silhouette, 4 blown white day, 5 caustic wall, 6 door cone, 8 lattice. mill-road is the eighth and is broken, not unlit - D-1a. **Judge room 3 on a strip, never a still** - the beacon flashes: three captures gave p95 55/95/56 |
| **D-1a** | Dioramas | Mill road renders black - **diagnosed, not lighting** | §4.3 | `OPEN` | Three probes, each reverted, each ruling something out. **Moon x7.4** (1.9→14): still 99.0% black, p95 0. **GUESS_FILL → #b0b0b0** (near white albedo): still 99.0%, p95 0. **A magenta `MeshBasicMaterial` cube, fog off, toneMapped off, placed at the shot's own target (0.15, 1.05, -6.5)**: zero magenta pixels. Nothing can hide that but the camera not being there or the props not being mounted - so it is NOT light, material, fog or the torchlight guess. Next: how SceneJump mounts tab 7 against the seven that work |
