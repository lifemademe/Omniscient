...
| **W-2** | Warehouse | Depth cueing — value + saturation falloff with distance | §4.4 | `OPEN` | **The next move is known and costed: hang the aisle bays higher.** They sit at 7.68m over a 10.5m roof, so the top load at 5.05m is 2.63m from a lamp against the floor's 7.68m and receives **7.7x what the floor does** — the vertical gradient across a rack is the room's biggest remaining flatness, and it is geometry, not tuning. Raising the emitter to ~8.98m needs intensity ×1.35 (700 → 945) and `distance` 9.6 → 10.7 to hold W-1's 5.8m floor pool; it cuts the top shelf to 63% while costing the floor nothing |
| **W-3** | Warehouse | A lamp on the drone that lights what it approaches | §4.4 | `IN LOOP` claude 2026-08-27 | Lamp 4.5 → **24**, emitter lifted and pushed forward. Three critics ran; each named a real gap and each was closed. Racking depolished (`RACK_PAINT`, metalness .58 → .08, albedo re-derived) — the blown band collapsed 94%. Cartons taken to a cardboard albedo (×0.78): a lamp-lit load body reads **222 against 179** for the brightest ambient-lit one, where a critic had measured **221 against 224** — a 3-level deficit became a 43-level lead. `W3-r6-cardboard.jpg` |
