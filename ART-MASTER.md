...
| **D-4** | Dioramas | One shadow-casting key per room | §5 | `IN LOOP` claude 2026-08-27 | **Audited all nine builders: exactly ONE had a shadow-casting key** (seedling tunnel). Eight rooms, 24 lights between them, nothing casting — which is why props sit ON a surface rather than IN a place. Repair shop done first: its `FaceKey` now casts. Chosen because it is the room's only SpotLight, so one shadow map rather than a point light's six-face cube, and it was already the key by intent. 1024 map, soft radius matched to its 0.72 penumbra. `D4-r0-before.jpg` / `D4-r1-shadow.jpg`. **Seven rooms still uncast** |
