...
| **C-3** | CRT | Phosphor persistence on state change | §4.2 | `IN LOOP` claude 2026-08-27 | Phosphor is an **asymmetry**, not a fade: a P1 grain strikes in microseconds and decays slowly, so symmetric easing is a dissolve and reads as software. CSS expresses it exactly, because the transition that runs is the one on the class being moved TO — brightening states get 40–45ms linear, dimming states get 300–340ms with a long tail. Applied to globe pins and console tabs; confirmed in the shipped bundle. **Honest limit: verified structurally, NOT in motion.** Triggering a pin state change or a tab switch needs a live contact session, which needs the globe-pin click that is still unsolved. A contact sheet through `record.py` would settle it in one pass |
