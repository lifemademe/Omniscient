...
| **W-3** | Warehouse | A lamp on the drone that lights what it approaches | §4.4 | `IN LOOP` claude 2026-08-26 | Lamp 4.5 → **24**, emitter lifted to -0.14 and pushed to -0.62. Parked at a BAY CENTRE, the load in front of the lamp reads 222 against 77 for an identical carton 5m left — **3.09x**, with 0.095% frame clipping all of it inside the lamp glass. Airframe passes clean. Racking given its own `RACK_PAINT` (metalness 0.58 → 0.08, colour re-derived to `#1e2d39`): the deck band's near-white p95 falls 222 → 191 with its median unchanged. `W3-r5-paint.jpg`, commit pending |
