...
| **D-4** | Dioramas | One shadow-casting key per room | §5 | `IN LOOP` claude 2026-08-27 | **Audit: exactly ONE of nine builders had a shadow-casting key.** Eight rooms, 24 lights, nothing casting — why props sit ON a surface rather than IN a place. Repair shop took **three rounds and two critic rejections**: (1) the narrow face spot cast onto the wall behind her and moved the radio by 0.01 levels; (2) the bench practical bought real contact and threw the lamp fixture's own silhouette across a third of the frame; (3) a `noShadowCast` opt-out in `applyShadowPolicy` exempts the fixture — pegboard back to 44.1 of its original 46.0, wedge 10.1 → 20.7, radio contact kept. **The lesson generalises: a small object close to a light casts an enormous shadow, so a practical's own fixture must not occlude it.** Seven rooms still uncast |
