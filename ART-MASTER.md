...
| **D-4** | Dioramas | One shadow-casting key per room | §5 | `IN LOOP` claude 2026-08-27 | **1 of 9 rooms had a key at the start; 6 do now.** Repair shop (4 rounds, 3 critic rejections), beacon mast, cleared house, flooded cellar, night door. **Three rules earned:** a key that cannot reach the surface objects rest on cannot create contact however strong it is; a fixture depicting a light must not occlude it — but exempt the shade, not the stand; and a directional costs one map where a point light costs a cube, which a diorama can afford and the warehouse cannot. **Mill road REVERTED** — the torch is the most motivated caster in the game and casting it took the scene to 98.8% of pixels under luma 10; exempting the torch body did not help, so it is not the fixture fault. Wire city and station desk have NO lights at all, so D-4 cannot apply as written. Captures `D4-tab2/5/6/7`, `D4-mast-after`, `D4-r4-base` |
